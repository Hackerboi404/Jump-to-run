...
# ====== BOT TOKEN & GAME URL ======
TOKEN = "8716842152:AAE_3JlzLZjr_Vgi9_Hax6rJBgmKAv5w0eQ"  # <-- BotFather se naya token lo
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Welcome! Use /game to play or /score to see leaderboard."
    )
...
async def score_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top_users:
        await update.message.reply_text("No one has played yet! Be the first 😉")
        return
...
async def web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"🔥 {user.first_name}, your score {score} has been saved!")
    except Exception as e:
        logging.error(f"Error handling web_app_data: {e}")
        await update.message.reply_text("⚠️ Something went wrong while saving your score.")
...
if __name__ == "__main__":
    init_db()
    threading.Thread(target=run_bot, daemon=True).start()  # Daemon thread for Render
